...
"""
python S1.py --config_file S1.yml
"""


sys.path.append('../..')

from data.basis import make_loaders, RandomReplDataCollator
from pytorch_lightning.loggers import CSVLogger, TensorBoardLogger
from models.mymodel import MacBertRandom4Csc
...
def args_parse(config_file=''):
    ...
    parser.add_argument(
        "--config_file", default="S1.yml", help="path to config file", type=str
    )
    parser.add_argument(
...
def main():
    ...
    collator = RandomReplDataCollator(tokenizer=tokenizer)
    # 加载数据
    ...
    if cfg.MODEL.WEIGHTS != '':
        if cfg.MODEL.NAME == 'macbert4csc':
            model = MacBertRandom4Csc.load_from_checkpoint(
        ...
        if cfg.MODEL.NAME == 'macbert4csc':
            model = MacBertRandom4Csc(cfg, tokenizer)
        else:
            raise ValueError("model not found.")
