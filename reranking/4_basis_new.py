...
for line in open('pre_top7_scores_6.txt', encoding='utf-8'):
    ...
    # 基于候选答案生成（预测特征，对比预测特征，优先级标签）
    for i in range(len(pres_sample)-1):
        # 如果当前样本是标准答案，生成一个联合所有属性的对比样本；反之，不生成
        if pres_sample[i]['text'] == tgt:
            for j in range(i+1, len(pres_sample)):
                # if random() > 0.5:
                m_dic = merge_dict_and_rename(pres_sample[i], pres_sample[j])
                m_dic['label'] = 1
                samples.append(m_dic)
                # else:
                d1 = copy.deepcopy(pres_sample[j])
                d2 = copy.deepcopy(pres_sample[i])
                m_dic = merge_dict_and_rename(d1, d2)
                m_dic['label'] = 0
                samples.append(m_dic)
...
print(df)
df.to_csv('./scores.csv', encoding='utf-8', index=False)
