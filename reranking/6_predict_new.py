...
import joblib
import os
from char_similar.char_similarity_std import sim_stroke, sim_component, cal_sim_by_pinyin, cal_sim_by_shape, sim_pinyin
...
def convert_candidates2features(_j):
    ...
    for index, p in enumerate(pres):
        ...
        for (k, v) in score:
            if 'f' in k or 'F' in k:
                sample['fluency'] = float(v)
            else:
                sample['semantic'] = float(v)
        print(_j)
        assert "fluency" in sample
        # if sample['fluency'] < 0.8:
        #     continue
        # if sample['semantic'] < 0.8:
        #     continue
...
for line in open('pre_top7_scores_6.txt', encoding='utf-8'):
    ...
    tgt = j['tgt']
    ori = j['ori']
    j_features = convert_candidates2features(j)
    for x in j_features:
        to_predict_l.append(x)

df = pd.DataFrame.from_records(to_predict_l)
print(df)
md5_df = df['md5']
# --------------------------- 预测 ---------------------------

# features = new_df[["phono_sim", "glyph_sim",'pinyin_sim','component_sim','freq_sim','stroke_sim', "semantic", "fluency"]]
features = df[['semantic', 'fluency','phono_sim', 'glyph_sim', 'pinyin_sim', 'component_sim', 'stroke_sim',
...
for model_name in models_name_set:
    ...
    with open(f'scores_{model_name}.json', 'w', encoding='utf-8') as f:
        ...
        for prediction, probability, md5 in zip(predictions, probabilities, md5_df.tolist()):
            print(prediction, probability[1], md5)
            out[md5] = probability[1]
